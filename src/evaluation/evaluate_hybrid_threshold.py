...
def main():
    fig_path = output_path("figures/hybrid_confusion_matrix_final.png")
    result_path = experiment_path("results/hybrid_final_metrics.txt")
    print_storage_summary({"figure": fig_path, "metrics": result_path})

    if len(sys.argv) > 1:
        threshold = float(sys.argv[1])
    else:
        threshold_file = experiment_path("results/hybrid_best_threshold.txt")
    ...
    model = HybridEEGNetTransformer().to(device)
    model.load_state_dict(
        torch.load(experiment_path("checkpoints/hybrid_best.pth"), map_location=device)
    )
    ...
    plt.title(f"Hybrid Confusion Matrix (thr={threshold:.2f})")
