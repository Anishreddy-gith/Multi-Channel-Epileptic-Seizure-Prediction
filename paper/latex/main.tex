...
Figure~\ref{fig:pipeline} summarizes the complete repository-backed workflow. Figures~\ref{fig:dataflow}, \ref{fig:preprocessing}, and \ref{fig:trainingpipeline} show the data flow, preprocessing, and training components used to organize the completed experiments.
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.92\textwidth]{data_flow_diagram.pdf}
\caption{Data flow from raw European Data Format (EDF) files to interim shards, patient-wise metadata splits, model checkpoints, metric files, and publication tables.}
\label{fig:dataflow}
\end{figure*}

\begin{figure*}[t]
\centering
\includegraphics[width=0.92\textwidth]{preprocessing_flowchart.pdf}
\caption{Frozen preprocessing workflow: required-channel check, channel ordering, 0.5--40 Hz bandpass filtering, 50 Hz notch filtering, 4 s windows with 2 s stride, per-window z-score normalization, and per-EDF shard writing.}
\label{fig:preprocessing}
\end{figure*}

\begin{figure*}[t]
\centering
\includegraphics[width=0.92\textwidth]{training_pipeline.pdf}
\caption{Training workflow combining patient-wise split metadata, lazy shard loading, model selection, AdamW optimization, mixed precision, early stopping, checkpointing, threshold selection, and final evaluation.}
...
Table~\ref{tab:split} gives the frozen patient-wise split. No patient appears in more than one split. This is essential because window-level random splits can leak patient-specific EEG characteristics and seizure-proximity patterns into validation or test data. Figures~\ref{fig:patientsplit}, \ref{fig:patientstats}, and \ref{fig:classdist} visualize the split and patient-level class distribution.
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{dataset_statistics_by_patient.pdf}
\caption{Window and label counts by patient. Patient-level variation motivates patient-wise reporting and conservative interpretation.}
\label{fig:patientstats}
\end{figure}
...
Table~\ref{tab:modelresults} reports the final model comparison. Figure~\ref{fig:modelcomp} visualizes validation PR-AUC and F1, while Figures~\ref{fig:roc}, \ref{fig:prcurve}, \ref{fig:confusion}, \ref{fig:learning}, and \ref{fig:valcurves} provide the final EEGNet evaluation curves.
...
\toprule
Model & Status & Val PR-AUC & Val F1 & Test PR-AUC & Test F1 \\
\midrule
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{roc_curve_eegnet.png}
\caption{Window-level ROC curve for the final EEGNet evaluation. ROC-AUC is secondary to PR-AUC because positives are rare.}
\label{fig:roc}
\end{figure}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{learning_curves_eegnet.png}
\caption{EEGNet learning curves from the completed run. The curves document optimization behavior but do not establish generalization beyond the frozen split.}
\label{fig:learning}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{validation_curves_eegnet.png}
\caption{EEGNet validation curves over training, used to inspect early stopping and threshold-dependent behavior.}
\label{fig:valcurves}
\end{figure}
...
Table~\ref{tab:horizon} reports the EEGNet preictal horizon ablation. Figures~\ref{fig:horizonpr}, \ref{fig:horizonf1}, and \ref{fig:horizonpositive} summarize PR-AUC, F1, and positive class size across horizons.
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{positive_class_size_vs_horizon.pdf}
\caption{Positive class size versus preictal horizon. Shortening the horizon monotonically reduces the number of positive windows.}
\label{fig:horizonpositive}
\end{figure}
